...
class EloquentController extends Controller
{
    public function index()
    {
       ...
       #dump(Admin::first());
        ...
        dump($admin);*/
         //软删除
      /*  $admin = Admin::find(6)->delete();
         dump($admin);*/
         //强制查询软删除数据 舍弃字deletd_at字段段的判断
        /*$admin = Admin::withTrashed()->find(3);
        dump($admin);*/
        //恢复数据  1、检索软删除模型（onlyTrashed）  2、强制查询软删除模型(withTrashed)
        //恢复  restore
       $amdin =  Admin::onlyTrashed()->find(6)->restore();
       dump($amdin);
       $amdin =  Admin::withTrashed()->find(6);
       dump($amdin);
        #$admin = Admin::restore()->find(3);
        #$admin = Admin::withTrashed()->where('id',3)->restore();
        # $admin = Admin::withTrashed()->find(3)->restore();
        # var_dump($admin);
        //强删除 1、 1、检索软删除模型（onlyTrashed）  2、强制查询软删除模型(withTrashed)
    ...
    /*    $data = Users::find(63)->userGroup->toArray();
        dump($data);*/
    //远程一对多
        $result  = Users::find(295)->many->toArray();
        dump($result);
    }
